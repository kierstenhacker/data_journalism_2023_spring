...
##What was happening when that positive relationship wasn't the case, who was successful on Spotify and then not YouTube
spotify_youtube %>%
...
collab_average_streams <- spotify_youtube %>%
  filter(artist == "Camila Cabello" | artist == "Lil Nas X" | artist == "Post Malone" | artist == "Dua Lipa" | artist == "Ed Sheeran" | artist == "Drake", str_detect(track,'feat.')) %>%
 group_by(artist) %>%
  summarise(
    mean_stream = mean(stream)
  )

non_collab_average_streams <- spotify_youtube %>%
  filter(artist == "Camila Cabello" | artist == "Lil Nas X" | artist == "Post Malone" | artist == "Dua Lipa" | artist == "Ed Sheeran" | artist == "Drake") %>%
 group_by(artist) %>%
  summarise(
    mean_stream = mean(stream)
  )
...
##check average streams for solo and collab to compare for a small list of artists to see who actually does well
##how would you restructure the data to make it actually useful
##are some of the views and streams low because the songs had jsut come out when the data was collected?
```
...
**There are only 10 songs for each artist that show up every time analysis is done**
