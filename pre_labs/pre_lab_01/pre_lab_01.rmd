...
**Answer: chr (character) and dbl (double) **
...
**Answer: organizing each row by the term and adding the total number of courses for each term in the second column in automatic ascending order**
...
**Answer: What changed is that we then arranged the data in the columns in descending order whereas above it was in the automatic ascending order**
...
**Answer: It is counting the number of classes by department and arranging it by term whereas the above code only arranged total classes by term and not department in descending order as well.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? 
**Answer: This codeblock now counts the number of seats and the total number of classes arranged by term which is different from the above codeblock with arranged by term and department and did not count the seats**
...
**Answer: We got by term the total number of classes, the total number of seats, the mean and median seats. It is similar to the previous code block in that it is arranged in descending order and grouped by term. It also counted classes and seats like above, but the mean and median number of seats are different in this codeblock. **
...
**Answer: This codeblock changed to group the data by department. In addition to showing the total seats, mean seat and median seats, the summary changed the codeblock to show the minimum and maximum seats. All of the columns are arranged by the total seat number in descending order.  **
...
1.  I am wondering if we could find out how many sections of classes there are so we could determine the average number of seats in a section of class for each department. I know prospective students are always interested in class sizes.

2.  I would also like to ask how the total number of classes offered per term has changed, or if there is a trend in the number of classes being offered in each department based on the term.

3.  Last, I would like to ask which department is offering the most seats or classes and how that may have changed from term to term. Is there a trend in which majors are gaining more popularity? Is that affecting the number of seats available or causing students to struggle to get into classes?
