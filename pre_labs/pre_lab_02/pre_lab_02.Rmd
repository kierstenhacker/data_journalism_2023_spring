...
**Answer: We lost information, so we can really ony use this function to show a very specific thing rather than showing what we need in addition to other helpful information.**
...
**Answer: 37,480. Or gives us both classes in the theater department and classes that have at least 15 seats. So, or shows us both things, but the data could be either one of them. And will show us data that is definitely both things defined in the filter.**
...
**Answer:It is listed in the year, month date fomrat. And in making the column into a data point we can perform other operations on it with filter and sourt.**
...
**Answer: floor date gave us an aggregate total by rounding the date down to be the first of the month so we were able to see how much was spent in each month during different years.**
...
**Answer: The code is adding the total votes for each candidate and then dividing Moore's total votes by the total of all votes to find the percentage. This code gives us the percent of total votes Moore received so then we can see his margin of winning.**
...
**Answer: This answer is different from the previous task because it is arranging Moore's percentage of votes in descending order. So, we can see where Moore won the most at the top and how badly he lost at the bottom. It shows also that Moore won the largest counties, but we also don't see what percentage of votes the other candidates or write-ins won.**
...
**Answer: This answer could be more interesting or useful for Moore if he decides to run for reelection to see where he could gain more votes. These results could also help Moore try to build rapport with those counties where he received a low percentage of votes.**
...
**Answer: str_to_lower converts to lower case, str_to_title converts to title case, str_which finds patterns and tells you which entries match, str_locate tells you the character positions the pattern characters are found in.**
...
**Answer: For each line of string detect, if R sees in the address that there is a pattern of the state name it will make a column where the state is abbreviated. It will separate the values specific to each state. For the last line, if there is something where the pattern of the state is not detected, it will make it N/A. The values are assigned to each of the states for multiple values.**
...
**Answer: Yes this answer makes sense because it would be highly likely that Maryland has more expenditures. The answer is also presented to us in a simple way that is easy to read so it understandable and it makes sense.**
...
1. I think now we could filter the data in different ways to ask how many courses have an instructor that says NA. In filtering for all the NA values, we could find a story that potentially holds the administration accountable for the lack of vital information they are providing students.

2.  Another question we could answer is whether there was a percent increase or decrease in total seats comparing two terms. For example, we could filter by department and then compare to see if there was a percent decrease in the number seats the departments offered in 2020 (pandemic) compareed to years prior. Or we could see if there was an increase after 2020.

3.  The last thing I can think of is to figure out how many classes are cross listed or listed twice under different names/course codes. We could also look to see if the same professors teach those courses. Or, we could also see if professors are listed as co-instructors adding to the courses they teach.
