...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer: No it does not show any parsing errors when the code is run.**
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer: The "Vendor Name" is a character, the "Amount" field is dbl, and the "Date" column is also character.**
...
**Answer: About 547 zip codes do not have five characters. What is happening is that the first 0 at the beginning of the zip code is getting dropped in the original data, leaving it with five characters.**
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer: There are NA values, there is an L and there is something called Contract in the category column. This could cause porblems by not including them in certain tasks due to the incorrect values.**
...
**Answer: There are two grants or loans that are less than $100 which seems really small and it also seems like it could be costing Maryland money. These values are defnitely outliers that could skew an average.**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer: The headers are changed to all lowercase with underscores instead of spaces in a new dataframe. The function changes them to be consistent and easy to work with.**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer: There are 50 possible duplicate rows in the dataframe. These could be duplicates or there could be multiple payments that that government processes in a way that makes it seem like they are duplicates. **
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer: The new dataframe has 17,740 rows, so half of the 50 duplicates are gone.**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer: The new column only has 5 digit zipcodes and no longer inlcudes the extra four digits on some of the entries.**
...
**Task** Write and run code to answer our original question: which zip code has gotten the most amount of money from the Maryland Tourism Board? **Answer: 21202 has gotten the most amount of money from the Maryland Tourism Board which makes sense since it is in Baltimore.**
