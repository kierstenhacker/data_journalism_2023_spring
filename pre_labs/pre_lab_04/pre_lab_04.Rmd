...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer: There are many organizations listed that are the same but spelled slighlty different so it prevents proper grouping since there are repeat organizations.**
...
**Answer: One change was from Charles County Arts Alliance Inc to CHARLES COUNTY ARTS ALLIANCE and another change was Montgomery County Chamber Comm Foundation Inc to MONTGOMERY COUNTY CHAMBER COMM FOUNDATION INC. **
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer: There aren't any changes until the third page when WASHINGTON ADVENTIST UNIVERSITY is changed to Washington Adventist University. So in the first 30 rows returned, only one change was made.  **
...
**Answer: No the changes do not vary substantially. There are still some repeat organizations that have different spelling and the rows have not been condensed any further. What is returned looks almost identical to the previous results **
