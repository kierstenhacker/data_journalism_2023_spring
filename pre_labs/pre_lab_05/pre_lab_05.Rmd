...
**Answer: The columns don't have proper headers on top and there are so many periods next to all the county/area names.**
...
**Answer: Without the headers it is still hard to read to decipher what each number is. There are also still periods after the areas and county names.**
...
**Answer: Now that the headers are fixed and we can read each year that the data is from and the jurisdiction there are still periods next to each jurisdiction which is another problem in the data.**
