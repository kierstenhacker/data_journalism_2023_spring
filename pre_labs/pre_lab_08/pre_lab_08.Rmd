...
**Answer: The first line of code is saving out a new dataframe called baltco_911_calls_by_month. In that dataframe, the second line of code is creating a month column using label=TRUE to make the months abbreviations rather than numbers. After that, the code is calculating the total calls in each month and arranging the totals from the highest to lowest total.**
...
**Answer: There is a blank gray rectangle that has created a canvas to make a chart/graph on.**
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer: After running the code, there is a bar chart visualizing the number of drug overdose 911 calls for each month. The x-axis so to say places the months in order, causing the bars to look a bit like a rollercoaster.**
...
**Answer: Now the bars are in order from the lowest number of calls in a month to the highest, rather than the order of months in the calander.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer: After running this codeblock, it has changed from the previous codeblock to switch the orientation from vertical to horizontal. Now the bars are horizontal with the highest number of calls on the top and the lowest number on the bottom.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer: This codeblock has changed the graph to get rid of the gray background and make it easier to read as well as more aesthetically pleasing.**
...
**Answer: This version of the chart is made to fit the economist theme from ggthemes. The chart has a blue-gray background with white horizontal lines behind the bars. The words on each axis have changed to black and so has the line representing the increments for the total count.**
```{r}
baltco_911_calls_by_month %>%
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer: This codeblock now adds a heading that describes the lead of the story at the top of the graph and it includes a source for the data at the bottom right. Each axis has also been relabeled with month and total count to help make the graph more readable/comprehensible.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer: This code shows us the total number of 911 drug overdose calls on particular dates in Baltimore County.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer: After running the code there is a line graph with the total number of calls for each date and it kind of looks like documentation of seismic activity or a crazy lie dectector test with lines all over the place, but we can still see the outliers.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer: After running the code there is a label and tick mark created for each date so we can see exactly when these fluctuations are happening. However, because there are so many dates they all overlap each other and it is hard to read/decipher them.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer: After running the code the date labels at the bottom are changed to be on an angle so they become easier to read even though there is still some overlap and the words are rather crowded.**
```{r}
baltco_911_calls_by_date %>%
  ggplot() + 
  geom_line(aes(x=date, y=total_calls)) + 
