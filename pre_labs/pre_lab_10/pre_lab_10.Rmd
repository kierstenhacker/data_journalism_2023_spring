...
**Answer: There are new columns we haven't seen before in this data that tell us the shape, area and length of a zip code area. There are also two columns for zip codes and a column for the town names in the zip codes.**
...
**Answer: This data includes information on zip codes and the towns contained in them as well as their geographic shape and size by longitude and latitude.**
...
**Answer: The output below is a blank map of Maryland outlining each zip code by latitude and longitude.**
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer: This code gives us data on every single county in the United States so we can use it to help create further maps with more detail. There are a lot of data points on the location, area and shape, especially in the geometry column again.**
```{r}
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer: This code returns just the Maryland counties out of all the U.S. using STATEFP as the identifier for Maryland.**
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer: This code makes a map similar to the map before but this time it plots counties by latitude and longitude rather than the zip code area. However, it includes the county longitude and latitude boundaries for the bay too, which makes it look like it is filled in with land rather than the actual shape of the state.**
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer: This map now contains a waterline to outline the bay and the river distinguishing it from the land. The bay still has outlines of the geographic areas that certain counties cover.**
...
**Task** Run the following code. Describe the output in the space below.
**Answer: The code loads data about foreclosures and returns a new dataframe that contains the amount of foreclosure notices issued in each zip code.** 
...
**Task** Run the following code. Describe the output in the space below.
**Answer: This code shows the geometry and the geographic area as well as the number of notices of foreclosures for each zip code.**
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer: This code gives us the map of Maryland with the number of foreclosure notices in each zip code shown by the color. The darker colors mean fewer foreclosures. We see that zip codes in the area of Prince George's and Charles County have higher numbers of foreclosures compared to counties in the rest of the state.**
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer: This code reads in data showing the date and the number of foreclosure notices in each county including a blank county at the bottom. Then it reads in acs population data, showing the population of each county.**
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer: Adding geometry = TRUE created a geometry column so now the dataframe has the population of each county as well as the shape by latitude and longitude points.**
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer: The columns for county do not match in order to be joined properly. We need to fix the county column data. The data shows the number of notices for each county and then also the population with the shape of the county.**
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer: This code renames the county and population column and removes Maryland from the counties. **
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer: This code joins the number of foreclosure notices by county with the population and includes the geometry column to make a map with the shapes of the counties.**
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer: This code calculates the rate of foreclosures per 1000 people by county in the dataframe that joined the population with the number of notices.**
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer: This map shows the rate of foreclosures per 1000 people in each county, using the lighter colors again to show a higher rate compared to the darker counties. There is the shape of each county in the map like we created before, but now it is filled with the color to represent the rate of foreclosures aesthetically.**
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer: The code changes the color theme of the scale to make it more readable and understandable since the blue shades are hard to decipher/tell apart. Now we can see the differences between the counties in a more clear manner. **
...
**Answer: This code changes the map to a log scale and adds a little more variation to the map so the distinction between the different rates in each county are easier to see. The map shows the pattern of the highest foreclosure rates in Charles County. Then Baltimore City, Prince George's and other counties on the eastern shore as well as western Maryland also have higher rates of foreclosres. So perhaps we see a pattern that rural areas or areas with more low-income communities experience higher rates of foreclosure notices.**
