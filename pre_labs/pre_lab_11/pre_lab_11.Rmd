...
**Answer: Now this code saves our data as an actual dataframe and the columns only show us results from 2023 which would be at the top of the list. There are two columns, one for the date and one for the title of the releases. The columns are both characters, even though one should really be a date, and neither have standardized names.** 
...
**Answer: It might be good to add a column specifying whether the release was a media advisory or a regular news release.** 
...
**Answer: Judging by the output, it suggests that there are 122 list tags in the HTML source. However, I am not seeing any unordered list tags (<ul>) since we isolated <li> tags but I am not sure if regular lists would fall under that.** 
...
**Answer: The output of this code shows us the text that is affiliated with every list tag item, but some text that is in the lists are not related to news releases at all, so they are not what we are looking for. Also, the output is not saved in a dataframe that would make it easier for us to look at and scroll through.** 
...
**Answer: The output gives us a dataframe with all the text from the html lists of press releases. This way we can work with the data to perfrom analysis. The dataframe still includes some of the list items that are not what we want to analyze and has the date on the same line as the title of the release in one column, which makes it confusing.** 
