...
**Answer: The dataframe has the date, title, url link and the text of each press release (and there's a lot of them) from Sen. Ben Cardin**
...
**Answer: Unique gives us the list of words without the repeats, defining each as its unique word in our data values.**
...
**Answer: unnest_tokens gives us a database with one column and 647091 rows of words in lowercase from unique words from our original database. **
...
**Answer: The graphic just shows that Cardin most commonly uses articles in the releases he posts. This isn't the most interesting since those words are extremely common in our everyday vocabulary.**
...
**Answer: Yes, the results are more interesting than before. The code gives us results of more unique words and how frequently they are used. The results also give us a percentage of the words used. These are all the words that aren't stop words that we defined as most used in the English language. Now there are more relevant results and interesting words that are more newsworthy. **
...
**Answer: The results give us the top ten words used in the press releases every year. In 2021, covid and words related to it are most common. In 2022 we start to shift away from covid words, but we still see a bit of it just now including more related to communities and Baltimore. In 2023, the words were much different, focusing on women, fire and other words that were not used very much at all in the other two years.**
...
**Task** Run the following code and describe the results.
**Answer: The results give us two word phrases that were most commonly used. The results make sense, with covid appearing a lot and other pandemic related phrases. There are some other Maryland focused words like Chesapeake Bay and Baltimore City. It just makes sense that covid has such a high percentage of usage, and I wonder if there would be even more if three words were included because then that means covid 19 pandemic would also be counted.**
...
**Task** Run the following code and describe the results.
**Answer: In 2022, we can see the shift away from covid 19 word usage in the results generated by the code below. Now there is more focus on projects and buildings/development as we see project location was used the most. Covid-19 and healthcare were still in the top ten phrases used, but not at the top anymore. These results give us the top 10 two word phrases used in 2022.**
...
**Task** Run the following code and describe the results.
**Answer: The results give us more evenly spread percentages of the top 10 words used in the press releases, given that there have only been about four months of them since the year is not over. We see a total shift in these results with human rights being the most used word which makes sense if we think about issues threatening human rights that have arisen both domestically and internationally. The next most used phrase was equal rights which also checks out due to the push for the ERA to be ratified. Overall, we see the top 10 most used phrases correlate with what is going on now rather than covid-19 like the two years before. **
...
**Answer: The results show us whether words tend to have a postive or negative connotation. However, some words were labeled one way or the other when in reality they could have both labels depending on the context. For example "work" or "significant" or even "important" could be related to postive or negative sentiments in a release depending on the subject matter. It is hard to assume whether these words are generally positive or negative without looking at the content of the releases as well, so it would not be an effective tool to rely on for storytelling or reporting, more would need to be done to look deeper into it.**
