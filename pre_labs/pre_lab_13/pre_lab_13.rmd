...
**Answer: The dataframe outputs a column for each state, the 2016 election candidates and the percent of votes won by Trump for every state. Then it also includes columns with the number of businesses that received PPP loans and the amount of PPP loan money per employee at the businesses.**
```{r}
...
**Answer: The scatter plot compares the percentage of votes won by Trump in each state to the amount of businesses who received PPP loans. It seems like there is a fairly clear positive relationship where states that Trump won received more PPP loans, although the dots are still a bit scattered. The line of best fit runs through the middle of the dots, with a gray outline indicating the spread.**
```{r}
...
```
It's a bit messy, but we can see something of a pattern here in the blob of dots.  Generally, the dots are moving from the lower left (less Trumpy states that got loans at a lower rate) to upper right (red states that got loans at a higher rate).  The blue "line of best fit" shows the general direction of the relationship. Let's test another variable, the average amount of money provided by PPP per small business employee in the state.

### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer: The relationship shown by this plot is a little less clear and not quite as strong. It is similar to the above plot with the percent of votes won by Trump on the x-axis but this time it is being compared with the amount of PPP loan money received per employee. Again, there is a bit of a positive relationship where the states where Trump won a higher percentage of votes also received more funding per employee. However, the dots are more spread out compared to the plot above and the relationship is not as strong/clear.**
```{r}
...
**Answer: The code gives us an NA for the percentage of votes won by Trump. Then it shows us the correlation coefficient between the vote percentage and the number of businesses that received loans and between the vote percentage and the amount of PPP loans per employee. The first coefficient shows that there is a somewhat positive relationship and the second one is not as strong of a positive relationship.**
...
**Answer: This code calculates the correlation coefficient and the p-value for statistical significance. The low p-value makes us feel more confident that these numbers are accurate and statistically significant making it meaningful to share with others in a possible story. The code also gives us the confidence interval and t-value.**
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer: This code calculates the same values as above, returning the p-value, t-value, correlation coefficient and confidence interval. The calculated p-value is high, making us less sure that the results are statistically significant so we would have less of a story about this relationship.**
```{r}
...
**Answer: The table shows values for each candidate. The columns give us the mean amount of PPP loan funding per employees for all of the states won by each candidate and the average number of businesses that received PPP loans also.**
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer: The output is similar in calculating the averages for us like above and giving us the p-value and t-value. The p-value is high so we should not have a lot of confidence that this is a meaningful relationship, especially to lead a story with. The code gives us the t-value, p-value, confidence interval and sample estimates for means for both Clinton and Trump. This is all for the amount of PPP loans per employee compared to the votes won.**
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer: Again, this code gives us a p-value and t-value as well as the averages. The p-value is much lower which means that the results are more statistically significant. The relationship is more likely to exist, it is not a product of random chance. This is more promising and we could write a story that a higher percentage of small businesses in states won by Trump received more loans before businesses in states won by Clinton. **
```{r}
